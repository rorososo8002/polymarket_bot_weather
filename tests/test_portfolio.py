...
def test_lock_only_high_exact_no_can_use_remaining_cash_when_settlement_return_clears(tmp_path):
    cfg = settings(
        tmp_path,
        bankroll_usd=1000.0,
        min_order_usd=10.0,
    ...
    client = FakeClient(
        {
            ...
            raw_market.no_token_id: OrderBook(
                ...
                bids=[OrderLevel(0.88, 2000.0)],
                asks=[OrderLevel(0.89, 2000.0)],
            ),
        }
    )
    ...
    assert decision.selected[0].result.size_usd > 900.0
