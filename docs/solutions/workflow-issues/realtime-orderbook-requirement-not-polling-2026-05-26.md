...
## Context
The bot requirement was to refresh forecasts every 30 minutes while monitoring Polymarket order books in real time. A short REST interval was still described as order-book monitoring, which violated the requirement and confused the production docs.
...
```text
Forecasts refresh every 30 minutes; order books use a short REST interval.
```

After:

```text
Forecasts refresh every 30 minutes; order books are monitored through WebSocket events.
```
